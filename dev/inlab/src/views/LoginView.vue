<template>
  <div class="login-container">
    ...
    <p>
      {{
        loginMode ? "Vous n'avez pas de compte ?" : "Vous avez déjà un compte ?"
      }}
...
export default {
  ...
  methods: {
    login() {
       ...
       * If the login is successful, stores the username and user ID in the session storage and redirects to the home page.
       * If there is an error during the login process, logs the error to the console.
       */
      const userData = {
      ...
      axios
        .post("http://localhost:5000/data/user/login", userData)
        .then((response) => {
          console.log(response.data);
          sessionStorage.setItem("username", this.username);
          // Ajouter l'ID de l'utilisateur à la session
          axios
            .get("http://localhost:5000/data/user/getUserId", {
              params: {
                username: this.username,
              },
            })
    ...
    signup() {
       ...
       */
      axios
        .post("http://localhost:5000/data/user/createUser", {
          username: this.username,
          password: this.password,
        })
        .then((response) => {
    ...
    toggleMode() {
      /**
       * Toggles the login mode between login and registration.
