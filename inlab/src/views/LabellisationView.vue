...
export default {
    ...
    methods: {
        ...
        async getPatch() {
            ...
            console.log(this.topLeftCoords);

            const imageWidth = image.getWidth();
            const imageHeight = image.getHeight();
            // Vérifiez si les valeurs i,j sont en dehors des limites de l'image
            if (this.i * this.patchSize > image.getWidth() || this.j * this.patchSize > image.getHeight()) {
            ...
            const patchName = `image_${this.i}_${this.j}.tiff`;
            const idImageSortie = 2;
            const patch = await image.readRasters({
                pool: pool,
                window: [offsetX, offsetY, offsetX + this.patchSize, offsetY + this.patchSize],
                interleave: true,
            });
            const metadata = await getImageMetadata(image, offsetX, offsetY, this.patchSize);
            // const left = metadata.ModelTiepoint[3];
            // const top = metadata.ModelTiepoint[4];
        ...
        traversePixelsInOrder(neighborsMap, width) {
            ...
            function dfs(pixel) {
                ...
                if (counter >= 10) {
                    ...
                    const [xStart, yStart] = [orderedPixels[0] % width, Math.floor(orderedPixels[0] / width)];

                    const [xCurrent, yCurrent] = [pixel % width, Math.floor(pixel / width)];
        ...
        bfs(labels, visited, i, j, width, height, region, neighbors) {
            ...
            // console.log(filteredConvPixels);
        ...
        flood_fill(labels, y, x, regionBoundaries) {
            ...
            const neighbors = Array.from(regionBoundaries.get(region) || new Set())
                .filter(neighbor => (regionBoundaries.get(neighbor) || new Set()).size === 1); // Filtrer les voisins ayant un seul voisin


            this.bfs(labels, visited, y, x, width, height, region, new Set(neighbors));
        },
