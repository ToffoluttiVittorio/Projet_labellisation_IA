...
function dfs(ctx: CanvasRenderingContext2D, labels: Uint32Array, visited: boolean[][], i: number, j: number, width: number, height: number, region: number) {
    ...
    if (i < 1 || i >= height - 1 || j < 1 || j >= width + 1 || region !== labels[i * width + j]) {
        console.log('mur')
        return;
    }
    ...
    dfs(ctx, labels, visited, i + 1, j, width, height, region);
    dfs(ctx, labels, visited, i - 1, j, width, height, region);
    dfs(ctx, labels, visited, i, j + 1, width, height, region);
    dfs(ctx, labels, visited, i, j - 1, width, height, region);
}
...
function flood_fill(ctx: CanvasRenderingContext2D, labels: Uint32Array, y: number, x: number, tiff: { width: number; height: number; channels: number; data: Uint8Array; }) {
    ...
    dfs(ctx, labels, visited, y, x, tiff.width, tiff.height, region);
